...
function updateTableTitle(title) {
  // TODO
  let tableTitle = document.querySelector("#table-title span");
  let node = document.createTextNode(title);
  tableTitle.textContent = "";
  tableTitle.appendChild(node);
}
...
function addContentToRow(child, row) {
  // TODO
  let element = createTableCell(child);
...
function buildRowForObservation(observation) {
  ...
  const time = createTime(observation.date.toLocaleDateString("fr-CA"));
