...
function titleCase(s) {
  ...
  if (typeof s == "string") {
    ...
    return str.join(" "); //convert into a string
  }
  return "";
}
...
function transformObservations(observations) {
  return observations.map(function (observation) {
    ...
    data.name =
      observation.taxon.preferred_common_name !== undefined
        ? titleCase(observation.taxon.preferred_common_name)
        : titleCase(observation.taxon.name);
    data.photoUrl = observation.taxon.default_photo.square_url;
    ...
    if (observation.taxon.endangered != null) {
      data.isEndangered = observation.taxon.endangered;
    } else {
      data.isEndangered = false;
    }
...
function filterObservations(observations) {
  return observations.filter(function (observation) {
    // TODO
    if (observation.taxon.default_photo != null && observation.taxon != null) {
      return observation;
    }
  });
}
